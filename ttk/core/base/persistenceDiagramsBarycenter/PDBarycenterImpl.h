...
std::vector<std::vector<matchingTuple>> PDBarycenter<dataType>::executePartialBiddingBarycenter(std::vector<diagramTuple>& barycenter){
	...
	stringstream timings_outputs;
	if(timings_plot_mode_){
	  timings_outputs << "energy\ttime\n";
	}
	double total_time = 0;
	...
	double time_real_cost_computation = 0;
        if(timings_plot_mode_){
            Timer t2;
            {
		...
		timings_outputs << real_total_cost <<"\t";
            }
            time_real_cost_computation = t2.getElapsedTime();
        }
		...
		if(!finished){
			...
			if(!use_progressive_ && epsilon<epsilon_0/500. && min_cost>total_cost){
				min_cost = total_cost;
				last_min_cost_obtained = 0;
			}
			else if(!use_progressive_ && epsilon<epsilon_0/500. ){
				last_min_cost_obtained += 1;
			}
 ...
 if(timings_plot_mode_){
  ofstream plot_file;
  plot_file.open("energy.txt", ios::out);
  plot_file << timings_outputs.str();
  plot_file.close();
